...
class Adapter(object):
    u"""
    The Adapter classes connect the templates to content. Note that an adapter always has a **Data**
    instance with attributes fields that fit the request. The caller needs to check if the requested fields
    really were filled. In case of an error the *article.error* field is holding the error message
    and the error fields are **None**. In case there is an unknown request, the output text is
    ...
    def getArticleIds(self, start=0, count=1, selector=None, order=None, **kwargs):
        ...
        numbers from **start** to **start+count**.
        The **selector** and **order** indicate the kind of articles to be
    ...
    def getArticle(self, id=None, index=0, selector=None, order=None, **kwargs):
        u"""Answer the article, indicate by **id**. If **id** is **None**
        or omitted, then try to find the id the sort order of article ids at **index**."""
        if id is None:
    ...
    def getArticles(self, ids=None, start=0, count=1, selector=None, order=None, **kwargs):
        ...
        of selection possible. **self.getArticles(count=4)** will answer the
        first 4 articles in the current sort order of articles. **self.getArticles(start=5, count=3)**
        will answer the articles with index 5, 6 and 7 in the current sort order of articles.
        **self.getArticles(ids=('aaa', 'bbb', 'ccc'))** will answer the articles with
